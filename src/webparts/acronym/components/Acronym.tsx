...
const Acronym: React.FC<IAcronymProps> = (props: IAcronymProps) => {
  ...
  const [letterMap, setLetterMap] = useState<Map<string, Map<string, string>>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    const getListData = async () => {
      ...
      const url =
        basePath +
        subsites +
        `_api/web/lists/GetByTitle('${parsedLink}')/items`;
      try {
        if (!props.spHttpClient)
          return console.error("fetch error", props.spHttpClient);
        const data = await props.spHttpClient
  ...
  if (!props.spListLink) return null;
